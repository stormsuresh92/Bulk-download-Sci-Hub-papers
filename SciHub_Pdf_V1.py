...
for doi in dois:
    try:
        ...
        r = s.get(base_url+doi.strip(), headers=header, timeout=8)
        cont = r.html.find('#article embed', first=True).attrs['src'].replace('#navpanes=0&view=FitH', '')
        if not cont.startswith('https:'):
            pdf = 'https:'+str(cont)
        else:
            pdf = cont
        time.sleep(5)
        output_file = open('PDF_Available.tsv', 'a')
        output_file.write(doi.strip() + '\t' + pdf + '\n')  
        output_file.close()
    
    except:
        output_file = open('No_PDFs.tsv', 'a')
